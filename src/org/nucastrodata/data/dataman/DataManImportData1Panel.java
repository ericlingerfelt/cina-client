...
/**
 * The Class DataManImportData1Panel.
 */
public class DataManImportData1Panel extends WizardPanel implements ItemListener{
	...
	//Declare panels
	/** The combo box panel. */
	JPanel mainPanel, reactionUIPanel, reactionTypeComboBoxPanel, notesPanel, comboBoxPanel; 
	...
	JComboBox reactionTypeComboBox, decayTypeComboBox;
	...
	JLabel arrowLabel, plusParenLabel, parenLabel, reactionTypeLabel, notesLabel, reactionNameLabel, topLabel, decayTypeLabel;
	...
	//Constructor
	/**
	 * Instantiates a new data man import data1 panel.
	 *
	 * @param ds the ds
	 */
	public DataManImportData1Panel(DataManDataStructure ds){
		...
		addWizardPanel("Nuclear Data Manager", "Import Nuclear Data", "1", "4");
		...
		topLabel = new JLabel("<html>Welcome to the Import Nuclear Data tool.<p><br>With this tool you can import and save nuclear data. "
								+ "In Step 1, choose a reaction<p>type and decay type from the dropdown menu, then enter the element and mass<p>"
								+ "number for each reactant and product in the fields below. Add notes for the nuclear<p>data in "
								+ "the <i>Notes</i> field.</html>");
	...
	public void setCurrentState(){
		...
		reactionTypeComboBox.setSelectedIndex(ds.getImportNucDataDataStructure().getReactionType()-1);
		
		//Call method setReactionStringUI to set up dynamic Z,A input fields
		setReactionStringUI(ds.getImportNucDataDataStructure().getReactionType()-1);
		
		decayTypeComboBox.setSelectedItem(ds.getImportNucDataDataStructure().getDecayType());
		...
		for(int i=0; i<numberReactants[reactionTypeComboBox.getSelectedIndex()]; i++){
			...
			reactionStringInZFieldArray[i].setText(String.valueOf((int)ds.getImportNucDataDataStructure().getIsoIn()[i].getX()));           
			...
			reactionStringInAFieldArray[i].setText(String.valueOf((int)ds.getImportNucDataDataStructure().getIsoIn()[i].getY()));
		...
		if(ds.getUseSecondProduct()){
			...
			for(int i=0; i<numberProducts[reactionTypeComboBox.getSelectedIndex()]; i++){
				...
				reactionStringOutZFieldArray[i].setText(String.valueOf((int)ds.getImportNucDataDataStructure().getIsoOut()[i].getX()));           
				reactionStringOutZFieldArray[i].setColumns(3);
				reactionStringOutAFieldArray[i].setText(String.valueOf((int)ds.getImportNucDataDataStructure().getIsoOut()[i].getY())); 
		...
		notesTextArea.setText(ds.getImportNucDataDataStructure().getNucDataNotes());
	...
	public void getCurrentState(){
		...
		ds.getImportNucDataDataStructure().setReactionType(reactionTypeComboBox.getSelectedIndex()+1);
		
		ds.getImportNucDataDataStructure().setDecayType((String)decayTypeComboBox.getSelectedItem());
		...
		//with the local one  
		ds.getImportNucDataDataStructure().setIsoIn(isoIn); 
		...
		if(ds.getUseSecondProduct()){
			...
			ds.getImportNucDataDataStructure().setNumberReactants(numberReactants[reactionTypeComboBox.getSelectedIndex()]);
			ds.getImportNucDataDataStructure().setNumberProducts(numberProducts[reactionTypeComboBox.getSelectedIndex()]);
			...
			ds.getImportNucDataDataStructure().setNumberReactants(numberReactants[reactionTypeComboBox.getSelectedIndex()]);
			ds.getImportNucDataDataStructure().setNumberProducts(1);
			
		}
		
		//Update the isoOut array of the data structure
		//with the local one
		ds.getImportNucDataDataStructure().setIsoOut(isoOut);
		
		//Update the data structure with the latest notes
		ds.getImportNucDataDataStructure().setNucDataNotes(notesTextArea.getText());
