...
/**
 * The Class DataMassChartPrintPanel.
 */
public class DataMassChartPrintPanel extends JPanel{
    ...
    /**
     * Instantiates a new data mass chart print panel.
     *
     * @param useWhiteText the use white text
     * @param ds the ds
     */
    public DataMassChartPrintPanel(boolean useWhiteText, DataMassDataStructure ds){
    
    	this.ds = ds;
    
    	setCurrentState(Cina.dataMassFrame.dataMassChartFrame.dataMassChartPanel.type
    						, Cina.dataMassFrame.dataMassChartFrame.dataMassChartPanel.quantity
    						, Cina.dataMassFrame.dataMassChartFrame.dataMassRainbowPanel.getScheme()
    						, useWhiteText);
	 ...
	 * @param useWhiteText the use white text
	 */
	public void setCurrentState(int type, int quantity, String scheme, boolean useWhiteText){
    ...
    public void paintComponent(Graphics g){
		...
		switch(type){
			...
			case 0:
				...
				g2.drawString("Theoretical Mass Model: " + ds.getTheoryModelDataStructure().getModelName(), 0, yLabel);
				
				break;
				
			case 1:
			
				g2.drawString("Reference Mass Model: " + ds.getExpModelDataStructure().getModelName(), 0, yLabel);
		...
		if(scheme.equals("Continuous")){
			...
			for(int i=0; i<200; i++){
				...
				g2.setColor(Cina.dataMassFrame.dataMassChartFrame.getRGB((double)i/200.0));
			...
			for(int i=0; i<200; i++){
				...
				double mag = (((maxTotal-minTotal)/200.0)*(double)i) + minTotal;
		...
		g2.drawString("Max: " + Cina.dataMassFrame.dataMassChartFrame.maxField.getText(), 71, 16);
		g2.drawString("Min: " + Cina.dataMassFrame.dataMassChartFrame.minField.getText(), 71, 200);
    ...
    public double getTitleScaleFactor(){
		...
		int length1 = reallyBigFontMetrics.stringWidth("Theoretical Mass Model: " 
								+ ds.getTheoryModelDataStructure().getModelName());
		
		int length2 = reallyBigFontMetrics.stringWidth("Reference Mass Model: " 
