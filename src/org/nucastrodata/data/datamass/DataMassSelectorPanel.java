...
/**
 * The Class DataMassSelectorPanel.
 */
public class DataMassSelectorPanel extends JPanel implements MouseListener, MouseMotionListener, Scrollable{
   ...
   static int[] minDripN;
    ...
    /**
     * Instantiates a new data mass selector panel.
     *
     * @param ds the ds
     */
    public DataMassSelectorPanel(DataMassDataStructure ds){
	...
	public void setLists(){
		...
		int length = ds.getSelectorModelDataStructure().getZNArray().length;

		int currentZ = -1;

		ZVector.addElement(new Integer(0));
		ZVector.addElement(new Integer(1));

		for(int i=0; i<length; i++){
		
			if(currentZ!=(int)ds.getSelectorModelDataStructure().getZNArray()[i].getX()){
			
				ZVector.addElement(new Integer((int)ds.getSelectorModelDataStructure().getZNArray()[i].getX()));
			
				currentZ = (int)ds.getSelectorModelDataStructure().getZNArray()[i].getX();
		...
		Vector[] NVector = new Vector[ZVector.size()];
		
		for(int i=0; i<ZVector.size(); i++){
		
			NVector[i] = new Vector();
		...
		for(int i=0; i<length; i++){
			...
			for(int j=0; j<ZVector.size(); j++){
				...
				currentZ = ((Integer)(ZVector.elementAt(j))).intValue();
				
				if(currentZ==(int)ds.getSelectorModelDataStructure().getZNArray()[i].getX()){
				
					NVector[j].addElement(new Integer((int)ds.getSelectorModelDataStructure().getZNArray()[i].getY()));
		...
		for(int i=0; i<ZVector.size(); i++){
			...
			NVector[i].trimToSize();
		
		}

		ZList = new int[ZVector.size()];
		isotopeList = new int[ZVector.size()][400];
		...
		for(int i=0; i<ZVector.size(); i++){
			...
			ZList[i] = ((Integer)(ZVector.elementAt(i))).intValue();

			for(int j=0; j<NVector[i].size(); j++){
			
				isotopeList[i][j] = ((Integer)(NVector[i].elementAt(j))).intValue();
	...
	public int[] getMaxDripN(){
		...
		for(int i=0; i<ZList.length; i++){
			...
			tempArray[i] = isotopeList[i][lastIndex];
    ...
    public void mousePressed(MouseEvent me) {
		...
		if((int)protonNumber>-1 && (neutronNumber>=minDripN[(int)protonNumber]) && (neutronNumber<=maxDripN[(int)protonNumber])){
			...
			if(Cina.rateManDataStructure.getIsotopeViktor().size()!=0){
    ...
    public void mouseEntered(MouseEvent me) {
    	...
    	Cina.dataMassFrame.dataMassSelectorFrame.ZRuler.setPreferredWidth((int)getSize().getWidth());
       	Cina.dataMassFrame.dataMassSelectorFrame.NRuler.setPreferredHeight((int)getSize().getHeight());
       	
       	Cina.dataMassFrame.dataMassSelectorFrame.ZRuler.setCurrentState(zmax, nmax, mouseX, mouseY, xoffset, yoffset, crosshairsOn);	
    	Cina.dataMassFrame.dataMassSelectorFrame.NRuler.setCurrentState(zmax, nmax, mouseX, mouseY, xoffset, yoffset, crosshairsOn);
    ...
    public void mouseExited(MouseEvent me) {
    	...
    	Cina.dataMassFrame.dataMassSelectorFrame.elementField.setText("");
    	Cina.dataMassFrame.dataMassSelectorFrame.zField.setText("");	
    	Cina.dataMassFrame.dataMassSelectorFrame.aField.setText("");
	...
	public void mouseMoved(MouseEvent me){
        ...
        if((int)protonNumber>-1 && (int)protonNumber<119 && (neutronNumber>=minDripN[(int)protonNumber]) && (neutronNumber<=maxDripN[(int)protonNumber])){
        	...
        	Cina.dataMassFrame.dataMassSelectorFrame.aField.setText(String.valueOf((int)neutronNumber + (int)protonNumber));
        
        	Cina.dataMassFrame.dataMassSelectorFrame.zField.setText(String.valueOf((int)protonNumber));
        
        	if((int)protonNumber>=2){
        	
		        Cina.dataMassFrame.dataMassSelectorFrame.elementField.setText(Cina.cinaMainDataStructure.getElementSymbol((int)protonNumber));
    ...
    public void paintComponent(Graphics g){
        ...
        for (int z=0; z<=zmax; z++){
            ...
            for(int n=minDripN[z]; n<=(int)Math.min((long)maxDripN[z],(long)nmax); n++){
			    ...
			    tempS = Cina.cinaMainDataStructure.getElementSymbol(z);
        ...
        for(int z=0; z<=zmax-1; z++){
		   ...
		   tempS = Cina.cinaMainDataStructure.getElementSymbol(zmax-z);
