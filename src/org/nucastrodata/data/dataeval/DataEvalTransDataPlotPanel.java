...
/**
 * The Class DataEvalTransDataPlotPanel.
 */
public class DataEvalTransDataPlotPanel extends PlotPanel implements MouseMotionListener, MouseListener{
	...
	/** The curve title. */
	String[] curveTitle = new String[2];
	...
	/** The original y. */
	boolean originalY = false;
	
	/** The original y2. */
	boolean originalY2 = false;
	...
	//Constructor
	/**
	 * Instantiates a new data eval trans data plot panel.
	 *
	 * @param yminInit the ymin init
	 * @param ymaxInit the ymax init
	 * @param ds the ds
	 */
	public DataEvalTransDataPlotPanel(int yminInit, int ymaxInit, DataEvalDataStructure ds){
	 ...
	 * Sets the original e.
	 *
	 * @param originalE the new original e
	 */
	 ...
	 * Sets the original y.
	 *
	 * @param originalY the new original y
	 */
	public void setOriginalY(boolean originalY){this.originalY = originalY;}
	
	/**
	 * Sets the original y2.
	 *
	 * @param originalY2 the new original y2
	 */
	public void setOriginalY2(boolean originalY2){this.originalY2 = originalY2;}
	
	/**
	 * Sets the final e.
	 *
	 * @param finalE the new final e
	 */
	 ...
	 * Sets the final y.
	 *
	 * @param finalY the new final y
	 */
	public void setFinalY(boolean finalY){this.finalY = finalY;}
	...
	public void setPlotState(){
		...
		if(ds.getTrans1NucDataDataStructure().getNucDataType().equals("CS(E)")){
			...
			titleString = "Cross Section for " + ds.getTrans1NucDataDataStructure().getReactionString();
			
		}else if(ds.getTrans1NucDataDataStructure().getNucDataType().equals("S(E)")){
		
			ytitle = "S Factor (keV-b)";
		
			titleString = "S Factor for " + ds.getTrans1NucDataDataStructure().getReactionString();
		...
		if(!ds.getTrans1NucDataDataStructure().getDecay().equals("")){
			...
			titleString += " [" + ds.getTrans1NucDataDataStructure().getDecay() + "]";
		...
		kmax = ds.getTrans1NucDataDataStructure().getNumberPoints()
				+ ds.getTrans2NucDataDataStructure().getNumberPoints();
		...
		if(initFlag){
			...
			xmin = Double.valueOf(Cina.dataEvalFrame.dataEvalTransData2Panel.EminField.getText()).doubleValue();
			xmax = Double.valueOf(Cina.dataEvalFrame.dataEvalTransData2Panel.EmaxField.getText()).doubleValue();
			
			ymin = 1e3;
			ymax = 1e7;
			
			ymin = Math.pow(10, Cina.dataEvalFrame.dataEvalTransData2Panel.getYmin());
			ymax = Math.pow(10, Cina.dataEvalFrame.dataEvalTransData2Panel.getYmax());
			
			ytickIntervals = Math.abs((int)Cina.dataEvalFrame.dataEvalTransData2Panel.getYmin()
									- (int)Cina.dataEvalFrame.dataEvalTransData2Panel.getYmax());
			...
			xmin = ds.getTransXmin();
			xmax = ds.getTransXmax();
		...
		if(ds.getTrans2NucDataDataStructure().getNumberPoints()==0){
		
			curveTitle[0] = "Original Curve: " + ds.getTrans1NucDataDataStructure().getNucDataName();
			curveTitle[1] = "";
			
			npoints[0] = ds.getTrans1NucDataDataStructure().getNumberPoints();

			for(int i=0; i<npoints[0]; i++){
				
				x[0][i] = ds.getTrans1NucDataDataStructure().getXDataArray()[i];
		
				y[0][i] = ds.getTrans1NucDataDataStructure().getYDataArray()[i];
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if(((String)Cina.dataEvalFrame.dataEvalTransData2Panel.scaleComboBox.getSelectedItem()).equals("Energy")){
			
				curveTitle[1] = "Gain Normalized Curve: " + ds.getTrans2NucDataDataStructure().getNucDataName();
			
			}else{
			
				curveTitle[1] = "Normalized Curve: " + ds.getTrans2NucDataDataStructure().getNucDataName();
			
			}
							
			npoints[0] = ds.getTrans1NucDataDataStructure().getNumberPoints();
			npoints[1] = ds.getTrans2NucDataDataStructure().getNumberPoints();
			...
			for(int i=0; i<npoints[1]; i++){
				...
				x[1][i] = ds.getTrans2NucDataDataStructure().getXDataArray()[i];
		
				y[1][i] = ds.getTrans2NucDataDataStructure().getYDataArray()[i];
	...
	public void paintComponent(Graphics g){
		...
		double currentOriginalE = Double.valueOf(Cina.dataEvalFrame.dataEvalTransData2Panel.originalEField.getText()).doubleValue();
		double currentFinalE = Double.valueOf(Cina.dataEvalFrame.dataEvalTransData2Panel.finalEField.getText()).doubleValue();

		double currentOriginalY = Double.valueOf(Cina.dataEvalFrame.dataEvalTransData2Panel.originalYField.getText()).doubleValue();
		double currentOriginalY2 = Double.valueOf(Cina.dataEvalFrame.dataEvalTransData2Panel.originalYField2.getText()).doubleValue();
		double currentFinalY = Double.valueOf(Cina.dataEvalFrame.dataEvalTransData2Panel.finalYField.getText()).doubleValue();

		currentOriginalY2 = log10*Math.log(currentOriginalY2);
		currentFinalY = log10*Math.log(currentFinalY);

		int wid = (x2-x1);
		int height = (y2-y1);
		
		double yymin = log10*Math.log(ymin);
        double yymax = log10*Math.log(ymax);
		
		double xscale = 1.0*(wid-xoffset-rightmarg)/(Math.abs(xmax-xmin));
		double yscale = 1.0*(height-yoffset-topmarg)/(yymax-yymin);
		
		int xPosOriginalE = (int)((currentOriginalE-xmin)*xscale);
		int xPosFinalE = (int)((currentFinalE-xmin)*xscale);

		int xPosOriginalY = (int)((currentOriginalY-xmin)*xscale);
		int yPosOriginalY2 = (int)((currentOriginalY2-yymin)*yscale);
		int yPosFinalY = (int)((currentFinalY-yymin)*yscale);

		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

		if(Cina.dataEvalFrame.dataEvalTransData2Panel.curve1Box.isSelected()
				|| Cina.dataEvalFrame.dataEvalTransData2Panel.curve2Box.isSelected()){
		...
		if(originalE){
	        ...
	        g2.drawLine(xPosOriginalE + xoffset + x1,y2 - yoffset,xPosOriginalE + xoffset + x1,y1 + topmarg); 
        ...
        if(finalE){
	        ...
	        g2.drawLine(xPosFinalE + xoffset + x1,y2 - yoffset,xPosFinalE + xoffset + x1,y1 + topmarg);
        ...
        if(originalY){
	        ...
	        g2.drawLine(xPosOriginalY + xoffset + x1,y2 - yoffset,xPosOriginalY + xoffset + x1,y1 + topmarg); 
        ...
        if(originalY2){
	        ...
	        g2.drawLine(x1 + xoffset,y2 - yoffset - yPosOriginalY2,x1 + wid - rightmarg,y2 - yoffset - yPosOriginalY2);
        ...
        if(finalY){
	        ...
	        g2.drawLine(x1 + xoffset,y2 - yoffset - yPosFinalY,x1 + wid - rightmarg,y2 - yoffset - yPosFinalY);
        ...
        if(showWindow && mouseDragging){
    		...
    		cinaStaticPlotter.plotIt(plotmode,newX1,newY1,newX2,newY2,
         	...
         	if(originalE){
		        ...
		        g2.drawLine(xPosOriginalE + xoffset + newX1,newY2 - yoffset,xPosOriginalE + xoffset + newX1,newY1 + topmarg); 
	        ...
	        if(finalE){
		        ...
		        g2.drawLine(xPosFinalE + xoffset + newX1,newY2 - yoffset,xPosFinalE + xoffset + newX1,newY1 + topmarg);
	        ...
	        if(originalY){
		        ...
		        g2.drawLine(xPosOriginalY + xoffset + newX1,newY2 - yoffset,xPosOriginalY + xoffset + newX1,newY1 + topmarg); 
	        ...
	        if(originalY2){
		        ...
		        g2.drawLine(newX1 + xoffset,newY2 - yoffset - yPosOriginalY2,newX1 + wid - rightmarg,newY2 - yoffset - yPosOriginalY2);
	        ...
	        if(finalY){
		        ...
		        g2.drawLine(newX1 + xoffset,newY2 - yoffset - yPosFinalY,newX1 + wid - rightmarg,newY2 - yoffset - yPosFinalY);
