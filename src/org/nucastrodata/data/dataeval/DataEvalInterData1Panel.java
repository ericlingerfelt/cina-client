...
/**
 * The Class DataEvalInterData1Panel.
 */
public class DataEvalInterData1Panel extends WizardPanel implements ActionListener, TreeExpansionListener{
	...
	//Constructor
	/**
	 * Instantiates a new data eval inter data1 panel.
	 *
	 * @param ds the ds
	 */
	public DataEvalInterData1Panel(DataEvalDataStructure ds){
		...
		addWizardPanel("Nuclear Data Evaluator's Toolkit", "Cross Section Fuser", "1", "3");
		...
		JLabel topLabel = new JLabel("<html>Welcome to the Cross Section Fuser tool.<p><br>With this tool you can combine two cross<p>section curves. "
								+ "In this step select the<p>cross section file for the first curve<p>from the tree at the right.</html>");
	...
	public void treeExpanded(TreeExpansionEvent tee){
		...
		if(tee.getPath().getPathCount()==5 
			...
			if(appndsdsTemp.getIsotopeList()[zIndex][aIndex]!=-1){
				...
				createNucDataLeaves(appndsdsTemp
	...
	public void actionPerformed(ActionEvent ae){
		...
		if(ae.getSource()==removeNucDataButton){
			...
			ds.getInter1NucDataDataStructure().setNucDataID("");
			...
			if(((DefaultMutableTreeNode)(setTree.getSelectionPath().getLastPathComponent())).isLeaf()){
				...
				if(!newNucData.equals("Only S-factor data is available")){
					...
					if(newNucData.indexOf("Reaction does not")==-1){
						...
						ds.getInter1NucDataDataStructure().setNucDataID(tempString);
	 ...
	 */
	private void createNucDataLeaves(NucDataSetDataStructure appndsds, DefaultMutableTreeNode parentNode, int zIndex, int aIndex){
	 ...
	 * @return true, if successful
	 */
	private boolean goodNucDataInfo(){
		...
		return Cina.cinaCGIComm.doCGICall("GET NUC DATA INFO", Cina.dataEvalFrame);
	...
	public void setCurrentState(){
		...
		nucDataField.setText(ds.getCurrentNucDataString1Eval());
		rateField.setText(ds.getCurrentRateString1Eval());
		nucDataSetField.setText(ds.getCurrentNucDataSetString1Eval());
	...
	public void getCurrentState(){
		...
		ds.setCurrentNucDataString1Eval(nucDataField.getText());
		ds.setCurrentRateString1Eval(rateField.getText());
		ds.setCurrentNucDataSetString1Eval(nucDataSetField.getText());
