...
/**
 * The Class DataEvalExtraDataPlotPanel.
 */
public class DataEvalExtraDataPlotPanel extends PlotPanel implements MouseMotionListener, MouseListener{
	...
	//Constructor
	/**
	 * Instantiates a new data eval extra data plot panel.
	 *
	 * @param yminInit the ymin init
	 * @param ymaxInit the ymax init
	 * @param ds the ds
	 */
	public DataEvalExtraDataPlotPanel(int yminInit, int ymaxInit, DataEvalDataStructure ds){
	 ...
	 * Sets the high e.
	 *
	 * @param highE the new high e
	 */
	public void setHighE(boolean highE){this.highE = highE;}
	
	/**
	 * Sets the low e.
	 *
	 * @param lowE the new low e
	 */
	public void setLowE(boolean lowE){this.lowE = lowE;}
	...
	public void setPlotState(){
		...
		if(ds.getExtra1NucDataDataStructure().getNucDataType().equals("CS(E)")){
			...
			titleString = "Cross Section for " + ds.getExtra1NucDataDataStructure().getReactionString();
			
		}else if(ds.getExtra1NucDataDataStructure().getNucDataType().equals("S(E)")){
		
			ytitle = "S Factor (keV-b)";
		
			titleString = "S Factor for " + ds.getExtra1NucDataDataStructure().getReactionString();
		...
		if(!ds.getExtra1NucDataDataStructure().getDecay().equals("")){
			...
			titleString += " [" + ds.getExtra1NucDataDataStructure().getDecay() + "]";
			
		}

		kmax = ds.getExtra1NucDataDataStructure().getNumberPoints()
				+ ds.getExtra2NucDataDataStructure().getNumberPoints();
				
		if(initFlag){

			showLegend = false;
	
			xmin = Double.valueOf(Cina.dataEvalFrame.dataEvalExtraData2Panel.EminField.getText()).doubleValue();
			xmax = Double.valueOf(Cina.dataEvalFrame.dataEvalExtraData2Panel.EmaxField.getText()).doubleValue();
			
			ymin = 1e3;
			ymax = 1e7;
			
			ymin = Math.pow(10, Cina.dataEvalFrame.dataEvalExtraData2Panel.getYmin());
			ymax = Math.pow(10, Cina.dataEvalFrame.dataEvalExtraData2Panel.getYmax());
			
			ytickIntervals = Math.abs((int)Cina.dataEvalFrame.dataEvalExtraData2Panel.getYmin()
									- (int)Cina.dataEvalFrame.dataEvalExtraData2Panel.getYmax());
									
			if(Cina.dataEvalFrame.dataEvalExtraData2Panel.curve1Box.isSelected()){
			...
			if(Cina.dataEvalFrame.dataEvalExtraData2Panel.curve2Box.isSelected()){
			...
			if(doplot[0]==1 && doplot[1]==1){
			
				lcolor[0] = Color.blue;
				lcolor[1] = Color.red;
			
			}else if(doplot[0]==0 && doplot[1]==1){
			
				lcolor[0] = Color.red;
				lcolor[1] = Color.red;
			
			}else if(doplot[0]==1 && doplot[1]==0){
			...
			xmin = ds.getExtraXmin();
			xmax = ds.getExtraXmax();
		...
		if(ds.getExtra2NucDataDataStructure().getNumberPoints()==0){
		
			curveTitle[0] = "";
			curveTitle[1] = "Original Curve: " + ds.getExtra1NucDataDataStructure().getNucDataName();
			
			npoints[0] = 0;
			npoints[1] = ds.getExtra1NucDataDataStructure().getNumberPoints();

			for(int i=0; i<npoints[1]; i++){
				
				x[1][i] = ds.getExtra1NucDataDataStructure().getXDataArray()[i];
		
				y[1][i] = ds.getExtra1NucDataDataStructure().getYDataArray()[i];
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			curveTitle[1] = "Original Curve: " + ds.getExtra1NucDataDataStructure().getNucDataName();
			curveTitle[0] = "Extrapolated Curve: " + ds.getExtra2NucDataDataStructure().getNucDataName();
			
			npoints[1] = ds.getExtra1NucDataDataStructure().getNumberPoints();
			npoints[0] = ds.getExtra2NucDataDataStructure().getNumberPoints();
			
			for(int i=0; i<npoints[0]; i++){
				
				x[0][i] = ds.getExtra2NucDataDataStructure().getXDataArray()[i];
		
				y[0][i] = ds.getExtra2NucDataDataStructure().getYDataArray()[i];
	...
	public void paintComponent(Graphics g){
		...
		double[] pointXArray = null;
		double[] pointYArray = null;
		
		if(highE){
		
			pointXArray = new double[Integer.valueOf(Cina.dataEvalFrame.dataEvalExtraData2Panel.numPointsHighEField.getText()).intValue()];
			pointYArray = new double[Integer.valueOf(Cina.dataEvalFrame.dataEvalExtraData2Panel.numPointsHighEField.getText()).intValue()];
		
			int counter = ds.getExtra1NucDataDataStructure().getNumberPoints()-1;
		
			for(int i=0; i<pointXArray.length; i++){
			
				pointXArray[i] = ds.getExtra1NucDataDataStructure().getXDataArray()[counter];
				pointYArray[i] = log10*Math.log(ds.getExtra1NucDataDataStructure().getYDataArray()[counter]);
			...
			pointXArray = new double[Integer.valueOf(Cina.dataEvalFrame.dataEvalExtraData2Panel.numPointsLowEField.getText()).intValue()];
			pointYArray = new double[Integer.valueOf(Cina.dataEvalFrame.dataEvalExtraData2Panel.numPointsLowEField.getText()).intValue()];
		
			for(int i=0; i<pointXArray.length; i++){
			
				pointXArray[i] = ds.getExtra1NucDataDataStructure().getXDataArray()[i];
				pointYArray[i] = log10*Math.log(ds.getExtra1NucDataDataStructure().getYDataArray()[i]);
		...
		double currentHighE = 0.0;
		int xPosHighE = 0;	
		
		double currentLowE = 0.0;
		int xPosLowE = 0;
	
		int psize = Math.max(3,dotSize);                        
    	int hpsize = psize/2;
	
		if(highE && !Cina.dataEvalFrame.dataEvalExtraData2Panel.highEField.getText().equals("NOT IN DOMAIN")){

			currentHighE = Double.valueOf(Cina.dataEvalFrame.dataEvalExtraData2Panel.highEField.getText()).doubleValue();
	
			xPosHighE = (int)((currentHighE-xmin)*xscale);	
	
		}
		
		if(lowE && !Cina.dataEvalFrame.dataEvalExtraData2Panel.lowEField.getText().equals("NOT IN DOMAIN")){
			
			currentLowE = Double.valueOf(Cina.dataEvalFrame.dataEvalExtraData2Panel.lowEField.getText()).doubleValue();

			xPosLowE = (int)((currentLowE-xmin)*xscale);

		}

		if(Cina.dataEvalFrame.dataEvalExtraData2Panel.curve1Box.isSelected()
				|| Cina.dataEvalFrame.dataEvalExtraData2Panel.curve2Box.isSelected()){
        ...
        if((lowE || highE) && Cina.dataEvalFrame.dataEvalExtraData2Panel.curve1Box.isSelected()){
        
	        for(int i=0; i<pointXArray.length; i++){
	
		          if(pointXArray[i] < xmin  ||
		             pointXArray[i] > xmax  ||
		             pointYArray[i] < yymin  ||
		             pointYArray[i] > yymax){continue;}
		             
		          int xi=(int)((pointXArray[i]-xmin)*xscale);
		          
		          int yi=(int)((pointYArray[i]-yymin)*yscale);
		          
		          g2.fillOval(x1+xoffset+xi-hpsize,y2-yi-hpsize-yoffset,psize,psize);
        ...
        if(highE && !Cina.dataEvalFrame.dataEvalExtraData2Panel.highEField.getText().equals("NOT IN DOMAIN")){
	        ...
	        g2.drawLine(xPosHighE + xoffset + x1,y2 - yoffset,xPosHighE + xoffset + x1,y1 + topmarg); 
        ...
        if(lowE && !Cina.dataEvalFrame.dataEvalExtraData2Panel.lowEField.getText().equals("NOT IN DOMAIN")){
	        ...
	        g2.drawLine(xPosLowE + xoffset + x1,y2 - yoffset,xPosLowE + xoffset + x1,y1 + topmarg);
    	...
    	if(showWindow && mouseDragging){
		        ...
		        for(int i=0; i<pointXArray.length; i++){
		          ...
		          if(pointXArray[i] < xmin  ||
		             ...
		             pointYArray[i] > yymax){continue;}
			          ...
			          g2.fillOval(newX1+xoffset+xi-hpsize,newY2-yi-hpsize-yoffset,psize,psize);
        	...
        	if(highE && !Cina.dataEvalFrame.dataEvalExtraData2Panel.highEField.getText().equals("NOT IN DOMAIN")){
		        ...
		        g2.drawLine(xPosHighE + xoffset + newX1,newY2 - yoffset,xPosHighE + xoffset + newX1,newY1 + topmarg); 
	        ...
	        if(lowE && !Cina.dataEvalFrame.dataEvalExtraData2Panel.lowEField.getText().equals("NOT IN DOMAIN")){
		        ...
		        g2.drawLine(xPosLowE + xoffset + newX1,newY2 - yoffset,xPosLowE + xoffset + newX1,newY1 + topmarg);
