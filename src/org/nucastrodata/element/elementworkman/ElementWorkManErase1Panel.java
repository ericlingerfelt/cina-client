...
public class ElementWorkManErase1Panel extends JPanel implements ActionListener{
	 ...
	 * @param ds the ds
	 */
	public ElementWorkManErase1Panel(ElementWorkManDataStructure ds){
		
		this.ds = ds;
		
		WordWrapLabel topLabel = new WordWrapLabel(true);
		topLabel.setText("<html>With the Erase Element Synthesis Workflows tool, you can delete element synthesis workflows from your User storage folder.</html>");
	...
	public boolean containsInUseWorkflows(){
		Iterator<ElementSimWorkDataStructure> itr = ds.getSimWorkMapSelected().values().iterator();
		while(itr.hasNext()){
			ElementSimWorkDataStructure eswds = itr.next();
			Cina.cgiCom.doCGICall(Cina.cinaMainDataStructure, eswds, CGICom.SIM_WORKFLOW_IN_USE, Cina.elementWorkManFrame);
			if(eswds.getInUse()){
				String string = "The element synthesis workflow " + eswds.getName() + " is currently in use by an offline simulation.";
				AttentionDialog.createDialog(Cina.elementWorkManFrame, string);
				return true;
			}
		}
		return false;
