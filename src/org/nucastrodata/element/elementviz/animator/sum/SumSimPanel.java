...
import org.nucastrodata.datastructure.feature.SumDataStructure.Time;
...
public class SumSimPanel extends JPanel implements ActionListener{
	...
	public SumSimPanel(SumDataStructure ds){
	...
	public boolean goodData(){
		...
		if(Integer.valueOf(amin)<0 || Integer.valueOf(amax)<0){
			return false;
		}
		
		if(Integer.valueOf(amin) > Integer.valueOf(amax)){
			return false;
		}
		
		if(Integer.valueOf(timestepmin) > Integer.valueOf(timestepmax)){
			return false;
		}
		
		if(Integer.valueOf(timestepmin)<0 
				|| Integer.valueOf(timestepmax)>(ds.getSim().getAbundTimestepDataStructureArray().length-1)){
			return false;
		}
	...
	public void calcSum(){
		...
		int aMin = Double.valueOf(ds.getA().getX()).intValue();
		int aMax = Double.valueOf(ds.getA().getY()).intValue();
		
		double sum = 0.0;
		
		for(int i=timestepMin; i<=timestepMax; i++){
			AbundTimestepDataStructure atsds = ds.getSim().getAbundTimestepDataStructureArray()[i];
			int[] indexArray = atsds.getIndexArray();
			for(int j=0; j<indexArray.length; j++){
				int a = Double.valueOf(ds.getSim().getZAMapArray()[indexArray[j]].getY()).intValue();
				if(a >= aMin && a <= aMax){
	...
	public void setCurrentState(){
		...
		if(ds.getTime()==SumDataStructure.Time.TIME_SEC){
			timesecButton.setSelected(true);
			timestepButton.setSelected(false);
		}else if(ds.getTime()==SumDataStructure.Time.TIME_STEP){
			timesecButton.setSelected(false);
			timestepButton.setSelected(true);
		}
		
		if(ds.getRange()==SumDataStructure.Range.CUSTOM){
			customButton.setSelected(true);
			defaultButton.setSelected(false);
		}else if(ds.getRange()==SumDataStructure.Range.DEFAULT){
		...
		timestepminField.setText(new Integer(ds.getTimeMap().get(SumDataStructure.Time.TIME_STEP).get(0).intValue()).toString());
		timestepmaxField.setText(new Integer(ds.getTimeMap().get(SumDataStructure.Time.TIME_STEP).get(1).intValue()).toString());
		timesecminField.setText(new PrintfFormat("%13.4E").sprintf(ds.getTimeMap().get(SumDataStructure.Time.TIME_SEC).get(0)).trim());
		timesecmaxField.setText(new PrintfFormat("%13.4E").sprintf(ds.getTimeMap().get(SumDataStructure.Time.TIME_SEC).get(1)).trim());
	...
	public void getCurrentState(){
		...
		if(timesecButton.isSelected()){
			ds.setTime(SumDataStructure.Time.TIME_SEC);
			...
			ds.setTime(SumDataStructure.Time.TIME_STEP);
		...
		timeMap.get(SumDataStructure.Time.TIME_STEP).add(new Double(timestepminField.getText()));
		timeMap.get(SumDataStructure.Time.TIME_STEP).add(new Double(timestepmaxField.getText()));
		timeMap.put(SumDataStructure.Time.TIME_SEC, new ArrayList<Double>());
		timeMap.get(SumDataStructure.Time.TIME_SEC).add(new Double(timesecminField.getText()));
		timeMap.get(SumDataStructure.Time.TIME_SEC).add(new Double(timesecmaxField.getText()));
 
    	ds.setTimeMap(timeMap);

		if(customButton.isSelected()){
			ds.setRange(SumDataStructure.Range.CUSTOM);
		}else if(defaultButton.isSelected()){
			ds.setRange(SumDataStructure.Range.DEFAULT);
		}
