...
/**
 * The Class ElementVizThermoPlotPanel.
 */
public class ElementVizThermoPlotPanel extends PlotPanel implements MouseMotionListener, MouseListener{
	 ...
	 * @param ds the ds
	 */
	public ElementVizThermoPlotPanel(int densityminInit
								, int densitymaxInit
	...
	public void setPlotState(){
		...
		x = ds.getTimeDataArrayThermo();

		if(initFlag){
						
			if(Cina.elementVizFrame.elementVizThermoPlotFrame.elementVizNucSimListPanel.getTempRadioButtonState()){

				y = ds.getTempDataArray();
			
			}else{
			
				y = ds.getDensityDataArray();
		...
		if(initFlag){
			...
			for(int i=0; i<size; i++){
				...
				npoints[i] = ds.getTimeDataArrayThermo()[i].length;
			...
			xmin = Cina.elementVizFrame.elementVizThermoPlotFrame.getTimemin();
			xmax = Cina.elementVizFrame.elementVizThermoPlotFrame.getTimemax();
			
			xtickIntervals = 10;
			
			if(Cina.elementVizFrame.elementVizThermoPlotFrame.elementVizNucSimListPanel.getTempRadioButtonState()){
			
				ymin = Cina.elementVizFrame.elementVizThermoPlotFrame.getTempmin();
				ymax = Cina.elementVizFrame.elementVizThermoPlotFrame.getTempmax();
		
				ydplace = 4;
		
				titleString = "Temperature vs. Time";
		
				ytitle = "Temperature (T9)";
			
				ytickIntervals = 10;
			
			}else{
			
				ymin = Math.pow(10, Cina.elementVizFrame.elementVizThermoPlotFrame.getDensitymin());
				ymax = Math.pow(10, Cina.elementVizFrame.elementVizThermoPlotFrame.getDensitymax());
				
				ydplace = 0;
				
				titleString = "Density vs. Time";
				
				ytitle = "Density (g/cm^3)";
				
				ytickIntervals = Math.abs((int)Cina.elementVizFrame.elementVizThermoPlotFrame.getDensitymin()
									- (int)Cina.elementVizFrame.elementVizThermoPlotFrame.getDensitymax());
			
			}
			
			minorX = Cina.elementVizFrame.elementVizThermoPlotFrame.getMinorX();
			majorX = Cina.elementVizFrame.elementVizThermoPlotFrame.getMajorX();			
			minorY = Cina.elementVizFrame.elementVizThermoPlotFrame.getMinorY();
			majorY = Cina.elementVizFrame.elementVizThermoPlotFrame.getMajorY();
			...
			for(int i=0; i<ds.getNucSimDataStructureArray().length; i++){
				...
				curveTitle[i] = Cina.elementVizFrame.elementVizThermoPlotFrame.elementVizNucSimListPanel.checkBoxArray[i].getText();
			
				if(Cina.elementVizFrame.elementVizThermoPlotFrame.elementVizNucSimListPanel.checkBoxArray[i].isSelected()){
				
					doplot[i] = 1;
					
					if(Cina.elementVizFrame.elementVizThermoPlotFrame.legendBox.isSelected()){
			...
			xmin = ds.getTimeminThermo();
			xmax = ds.getTimemaxThermo();
