...
public class ElementManCopy1Panel extends JPanel implements ActionListener{
	 ...
	 * @param ds the ds
	 */
	public ElementManCopy1Panel(ElementManDataStructure ds){
		
		this.ds = ds;
		
		WordWrapLabel topLabel = new WordWrapLabel(true);
		topLabel.setText("<html>With the Copy Element Synthesis Simulation to Shared Folder tool, you can copy "
				 + "element synthesis simulations from your User storage folder to the Shared storage folder. "
				 + "Element synthesis simulations in the Shared storage folder can be accessed by all Users of the suite.<br><br>"
				 + "Contact coordinator@nucastrodata.org if you wish to remove an element synthesis simulation "
	...
	public boolean sharedSimExists(){
		for(int i=0; i<listModel.size(); i++){
			ElementSimDataStructure mmds = (ElementSimDataStructure)listModel.get(i);
			if(ds.getSimMap().containsKey("SHARED/" + mmds.getName())){
				return true;
			}
		}
		return false;
