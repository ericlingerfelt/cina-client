...
/**
 * The Class RateManAboutRate1Panel.
 */
public class RateManAboutRate1Panel extends WizardPanel implements ActionListener, TreeExpansionListener{
	...
	/** The used tree paths. */
	private Vector rateIDVector, homeLibVector, decayVector, usedTreePaths;
	...
	//Constructor
	/**
	 * Instantiates a new rate man about rate1 panel.
	 *
	 * @param ds the ds
	 */
	public RateManAboutRate1Panel(RateManDataStructure ds){
		...
		addWizardPanel("Rate Manager", "Rate Info", "1", "2");
		
		JPanel treePanel = new JPanel(new BorderLayout());
		
		JLabel treeTitleLabel = new JLabel("Select reactions from the tree below.");
		JLabel listTitleLabel = new JLabel("Rate list");
	...
	public void actionPerformed(ActionEvent ae){
		...
		if(ae.getSource()==removeRateButton){
			...
			if(reactionList.getSelectedValues().length > 0){
	...
	protected boolean emptyRates(){
		...
		if(ds.getRateIDVectorAboutRate().size()!=0){
			return false;
		}
		return true;
	...
	public void setCurrentState(){
		...
		for(int i=0; i<getRateList().length; i++){reactionListModel.addElement(getRateList()[i]);}
		
		rateIDVector = ds.getRateIDVectorAboutRate();
		decayVector = ds.getDecayVectorAboutRate();
		homeLibVector = ds.getHomeLibVectorAboutRate();
	...
	private String[] getRateList(){
		...
		String[] stringArray = new String[ds.getRateIDVectorAboutRate().size()];
		
		for(int i=0; i<stringArray.length; i++){
		
			String rateID = (String)ds.getRateIDVectorAboutRate().elementAt(i);
			StringTokenizer st09 = new StringTokenizer(rateID, "\u0009");
			String dummy = st09.nextToken().substring(8);
			String library = (String)ds.getHomeLibVectorAboutRate().elementAt(i);
			String tempString = st09.nextToken();
			String reactionString = tempString.substring(0, tempString.indexOf("\u000b"));
			String decay = (String)ds.getDecayVectorAboutRate().elementAt(i);
	...
	public void getCurrentState(){
		...
		ds.setRateIDVectorAboutRate(rateIDVector);
		
		decayVector.trimToSize();
		ds.setDecayVectorAboutRate(decayVector);
	
		homeLibVector.trimToSize();
		ds.setHomeLibVectorAboutRate(homeLibVector);
