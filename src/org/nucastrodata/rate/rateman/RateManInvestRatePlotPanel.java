...
/**
 * The Class RateManInvestRatePlotPanel.
 */
public class RateManInvestRatePlotPanel extends PlotPanel implements MouseMotionListener, MouseListener{
	...
	/**
	 * Instantiates a new rate man invest rate plot panel.
	 *
	 ...
	 * @param ds the ds
	 */
	public RateManInvestRatePlotPanel(int rminInit
								, int rmaxInit
								, int tminInit
								, int tmaxInit
	...
	public void setPlotState(){
		...
		imax = ds.getNumberTotalRates()
							+ ds.getNumberCompRates();
	
		mode = new int[ds.getNumberTotalRates()
							+ ds.getNumberCompRates()];
							
		doplot = new int[ds.getNumberTotalRates()
							+ ds.getNumberCompRates()];
							
		curveTitle = new String[ds.getNumberTotalRates()
							+ ds.getNumberCompRates()];
		
		x = new double[ds.getNumberTotalRates() 
						+ ds.getNumberCompRates()][ds.getTempGrid().length];
		
		y = new double[ds.getNumberTotalRates() 
						+ ds.getNumberCompRates()][ds.getTempGrid().length];
    	...
    	for(int i=0; i<ds.getInvestRateVectorArray().length; i++){
    		...
    		if(rateUsed){
				...
				x[numberRates] = ((RateDataStructure)ds.getInvestRateVectorArray()[i].elementAt(0)).getTempDataArrayTotal();
				y[numberRates] = ((RateDataStructure)ds.getInvestRateVectorArray()[i].elementAt(0)).getRateDataArrayTotal();
				...
				if(((RateDataStructure)ds.getInvestRateVectorArray()[i].elementAt(0)).getResonantInfo().length>1){
					...
					for(int j=0; j<((RateDataStructure)ds.getInvestRateVectorArray()[i].elementAt(0)).getResonantInfo().length; j++){
						...
						x[numberRates] = ((RateDataStructure)ds.getInvestRateVectorArray()[i].elementAt(0)).getTempDataArrayComp()[j];
						y[numberRates] = ((RateDataStructure)ds.getInvestRateVectorArray()[i].elementAt(0)).getRateDataArrayComp()[j];
		...
		if(initFlag){
			...
			plotmode = Cina.rateManFrame.rateManInvestRatePlotFrame.getPlotMode();
		
			xmin = Math.pow(10, Cina.rateManFrame.rateManInvestRatePlotFrame.getTmin());
			xmax = Math.pow(10, Cina.rateManFrame.rateManInvestRatePlotFrame.getTmax());
			ymin = Math.pow(10, Cina.rateManFrame.rateManInvestRatePlotFrame.getRmin());
			ymax = Math.pow(10, Cina.rateManFrame.rateManInvestRatePlotFrame.getRmax());
			
			minorX = Cina.rateManFrame.rateManInvestRatePlotFrame.getMinorT();
			majorX = Cina.rateManFrame.rateManInvestRatePlotFrame.getMajorT();			
			minorY = Cina.rateManFrame.rateManInvestRatePlotFrame.getMinorR();
			majorY = Cina.rateManFrame.rateManInvestRatePlotFrame.getMajorR();
			
			xtickIntervals = Math.abs((int)Cina.rateManFrame.rateManInvestRatePlotFrame.getTmin()
									- (int)Cina.rateManFrame.rateManInvestRatePlotFrame.getTmax());
		
			ytickIntervals = Math.abs((int)Cina.rateManFrame.rateManInvestRatePlotFrame.getRmin()
									- (int)Cina.rateManFrame.rateManInvestRatePlotFrame.getRmax());
	
			showLegend = false;

			for(int i=0; i<ds.getNumberTotalRates()
							+ ds.getNumberCompRates(); i++){

				if(Cina.rateManFrame.rateManInvestRatePlotFrame.rateManInvestRatePlotListPanel.checkBoxArray[i].getText().indexOf("(nr")!=-1
					|| Cina.rateManFrame.rateManInvestRatePlotFrame.rateManInvestRatePlotListPanel.checkBoxArray[i].getText().indexOf("(r")!=-1){
				...
				curveTitle[i] = Cina.rateManFrame.rateManInvestRatePlotFrame.rateManInvestRatePlotListPanel.checkBoxArray[i].getText();
		...
		npoints = new int[ds.getNumberTotalRates()
							+ ds.getNumberCompRates()];
