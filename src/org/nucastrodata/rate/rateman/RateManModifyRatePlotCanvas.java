...
/**
 * The Class RateManModifyRatePlotCanvas.
 */
public class RateManModifyRatePlotCanvas extends JPanel implements MouseMotionListener, MouseListener{
	...
	//Constructor
	/**
	 * Instantiates a new rate man modify rate plot canvas.
	 *
	 ...
	 * @param ds the ds
	 */
	public RateManModifyRatePlotCanvas(int rminInit
										, int rmaxInit
										, int tminInit
										, int tmaxInit
	...
	public void setPlotState(){
		...
		if(ds.getModifyRateDataStructure().getResonantInfo().length>1){
			...
			x = new double[ds.getModifyRateDataStructure().getResonantInfo().length+1][ds.getTempGrid().length];
		
			y = new double[ds.getModifyRateDataStructure().getResonantInfo().length+1][ds.getTempGrid().length];
		...
		for(int i=0; i<ds.getTempGrid().length; i++){
			...
			x[0][i] = ds.getModifyRateDataStructure().getTempDataArrayTotal()[i];
			
			y[0][i] = ds.getModifyRateDataStructure().getRateDataArrayTotal()[i];
		...
		if(ds.getModifyRateDataStructure().getResonantInfo().length>1){
			...
			for(int i=1; i<ds.getModifyRateDataStructure().getResonantInfo().length+1; i++){

				for(int j=0; j<ds.getTempGrid().length; j++){

					x[i][j] = ds.getModifyRateDataStructure().getTempDataArrayComp()[i-1][j];
			
					y[i][j] = ds.getModifyRateDataStructure().getRateDataArrayComp()[i-1][j];
		...
		titleString = ds.getModifyRateDataStructure().getReactionString()
		  				+ " ("
		...
		curveTitle[0] = titleString + units[ds.getModifyRateDataStructure().getReactionType()];
	
		for(int i=1; i<numberCurves; i++){
			
			curveTitle[i] = titleString 
								+ " ("
								+ ds.getModifyRateDataStructure().getResonantInfo()[i-1]
								+ ")";
		...
		if(initFlag){
			...
			xmin = Math.pow(10, Cina.rateManFrame.rateManModifyRatePlotFrame.getTmin());
			xmax = Math.pow(10, Cina.rateManFrame.rateManModifyRatePlotFrame.getTmax());
			ymin = Math.pow(10, Cina.rateManFrame.rateManModifyRatePlotFrame.getRmin());
			ymax = Math.pow(10, Cina.rateManFrame.rateManModifyRatePlotFrame.getRmax());
			
			minorX = Cina.rateManFrame.rateManModifyRatePlotFrame.getMinorT();
			majorX = Cina.rateManFrame.rateManModifyRatePlotFrame.getMajorT();			
			minorY = Cina.rateManFrame.rateManModifyRatePlotFrame.getMinorR();
			majorY = Cina.rateManFrame.rateManModifyRatePlotFrame.getMajorR();
			
			xtickIntervals = Math.abs((int)Cina.rateManFrame.rateManModifyRatePlotFrame.getTmin()
									- (int)Cina.rateManFrame.rateManModifyRatePlotFrame.getTmax());
		
			ytickIntervals = Math.abs((int)Cina.rateManFrame.rateManModifyRatePlotFrame.getRmin()
									- (int)Cina.rateManFrame.rateManModifyRatePlotFrame.getRmax());
		
			
		}else{
		
			ytitle = ytitle + " (" + units[ds.getModifyRateDataStructure().getReactionType()] + ")";
			initFlag = true;
