...
import org.nucastrodata.rate.rateman.RateManModifyRatePlotCanvas;
import org.nucastrodata.rate.rateman.RateManModifyRatePlotPrintable;


/**
 * The Class RateManModifyRatePlotFrame.
 */
public class RateManModifyRatePlotFrame extends JFrame implements ActionListener, WindowListener, ItemListener{

	/** The gbc. */
	GridBagConstraints gbc;
	
	/** The rate man modify rate plot canvas. */
	RateManModifyRatePlotCanvas rateManModifyRatePlotCanvas;
	...
	/**
	 * Instantiates a new rate man modify rate plot frame.
	 *
	 * @param ds the ds
	 */
	public RateManModifyRatePlotFrame(RateManDataStructure ds){
	...
	public void setFormatPanelState(){
		...
		if(ds.getModifyRateDataStructure().getDecay().equals("")){
			...
			setTitle("Modify Existing Rate Plot: " 
	...
	public void windowClosing(WindowEvent we) {  
		...
		Cina.rateManFrame.rateManModifyRate3Panel.plotButton.setText("<html>Open Plotting<p>Interface</html>");
	...
	public void createResultsPlot(){
		...
		rateManModifyRatePlotCanvas = new RateManModifyRatePlotCanvas(rminComboBoxInit 
																	, rmaxComboBoxInit
																	, tminComboBoxInit
																	, tmaxComboBoxInit
																	, ds);
		
		rateManModifyRatePlotCanvas.setPreferredSize(rateManModifyRatePlotCanvas.getSize());
		
		rateManModifyRatePlotCanvas.revalidate();
		
		sp = new JScrollPane(rateManModifyRatePlotCanvas);
		
		JViewport vp = new JViewport();
		
		vp.setView(rateManModifyRatePlotCanvas);
	...
	public int setInitialrminComboBoxInit(){
		...
		currentrmin = ds.getModifyRateDataStructure().getRatemin();
	...
	public int setInitialrmaxComboBoxInit(){
		...
		currentrmax = ds.getModifyRateDataStructure().getRatemax();
	...
	public int setInitialtminComboBoxInit(){
		...
		currenttmin = ds.getModifyRateDataStructure().getTempmin();
	...
	public int setInitialtmaxComboBoxInit(){
		...
		currenttmax = ds.getModifyRateDataStructure().getTempmax();
	...
	public void actionPerformed(ActionEvent ae){
		...
		if(ae.getSource()==printButton){
			...
			PlotPrinter.print(new RateManModifyRatePlotPrintable(), this);
		
		}else if(ae.getSource()==saveButton){
			
			PlotSaver.savePlot(rateManModifyRatePlotCanvas, this);
	...
	public void redrawPlot(){
		...
		if(Integer.valueOf(rminComboBox.getSelectedItem().toString()).intValue() 
			...
			rateManModifyRatePlotCanvas.setPlotState();
			rateManModifyRatePlotCanvas.repaint();
...
class RateManModifyRatePlotPrintable implements Printable{

	public int print(Graphics g, PageFormat pf, int pageIndex){
	
		if(pageIndex!=0){return NO_SUCH_PAGE;}
		
		Cina.rateManFrame.rateManModifyRatePlotFrame.rateManModifyRatePlotCanvas.paintMe(g);
