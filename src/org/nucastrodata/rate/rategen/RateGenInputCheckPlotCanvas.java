...
/**
 * The Class RateGenInputCheckPlotCanvas.
 */
public class RateGenInputCheckPlotCanvas extends JPanel implements MouseMotionListener, MouseListener{
	...
	//Constructor
	/**
	 * Instantiates a new rate gen input check plot canvas.
	 *
	 * @param yminInit the ymin init
	 * @param ymaxInit the ymax init
	 * @param ds the ds
	 */
	public RateGenInputCheckPlotCanvas(int yminInit
										, int ymaxInit
	...
	public void setPlotState(){
		...
		for(int i=0; i<kmax; i++){
			...
			x[0][i] = ds.getEnergyDataArray()[i];

			if(ds.getInputType().equals("CS(E)")){
			
				y[0][i] = ds.getCrossSectionDataArray()[i];
			
			}else if(ds.getInputType().equals("S(E)")){
			
				y[0][i] = ds.getSFactorDataArray()[i];
		...
		if(initFlag){
			...
			ymin = Math.pow(10, Cina.rateGenFrame.rateGenInputCheckPlotFrame.getYmin());
			ymax = Math.pow(10, Cina.rateGenFrame.rateGenInputCheckPlotFrame.getYmax());
			
			ytickIntervals = Math.abs((int)Cina.rateGenFrame.rateGenInputCheckPlotFrame.getYmin()
									- (int)Cina.rateGenFrame.rateGenInputCheckPlotFrame.getYmax());
	 ...
	 */
	public void initializeEnergyPlot(){
		
 		plotmode = 1;

		if(Cina.rateGenFrame.rateGenInputCheckPlotFrame.EBox.isSelected()){
		...
		xmin = Double.valueOf(Cina.rateGenFrame.rateGenInputCheckPlotFrame.EminField.getText()).doubleValue();
		xmax = Double.valueOf(Cina.rateGenFrame.rateGenInputCheckPlotFrame.EmaxField.getText()).doubleValue();
	...
	/**
	 * Initialize energy plot init.
	 */
	public void initializeEnergyPlotInit(){
	 ...
	 * @return the double
	 */
	public double findXmin(){
		
		double newXmin = 1e30;
		
		for(int i=0; i<kmax; i++){
			
			newXmin = Math.min(newXmin, x[0][i]);
	 ...
	 * @return the double
	 */
	public double findXmax(){
		
		double newXmax = 0.0;
		
		for(int i=0; i<kmax; i++){
			
			newXmax = Math.max(newXmax, x[0][i]);
