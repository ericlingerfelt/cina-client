...
import org.nucastrodata.rate.rategen.RateGenResultsPlotCanvas;
import org.nucastrodata.rate.rategen.RateGenResultsPlotPrintable;


/**
 * The Class RateGenResultsPlotFrame.
 */
public class RateGenResultsPlotFrame extends JFrame implements ActionListener, WindowListener, ItemListener{

	/** The gbc. */
	GridBagConstraints gbc;
	
	/** The rate gen results plot canvas. */
	RateGenResultsPlotCanvas rateGenResultsPlotCanvas;
	...
	/**
	 * Instantiates a new rate gen results plot frame.
	 *
	 * @param ds the ds
	 */
	public RateGenResultsPlotFrame(RateGenDataStructure ds){
	...
	public void createResultsPlot(){
		...
		rateGenResultsPlotCanvas = new RateGenResultsPlotCanvas(rminComboBoxInit 
																	, rmaxComboBoxInit
																	, tminComboBoxInit
																	, tmaxComboBoxInit
																	, ds);
		
		rateGenResultsPlotCanvas.setPreferredSize(rateGenResultsPlotCanvas.getSize());
		
		rateGenResultsPlotCanvas.revalidate();
		
		sp = new JScrollPane(rateGenResultsPlotCanvas);
		
		JViewport vp = new JViewport();
		
		vp.setView(rateGenResultsPlotCanvas);
	...
	public int setInitialrminComboBoxInit(){
		...
		currentrmin = ds.getRatemin();
		
		min = (int)Math.floor(0.434294482*Math.log(currentrmin));
	...
	public int setInitialrmaxComboBoxInit(){
		...
		currentrmax = ds.getRatemax();
		
		max = (int)Math.ceil(0.434294482*Math.log(currentrmax));
	...
	public void actionPerformed(ActionEvent ae){
		...
		if(ae.getSource()==printButton){
			...
			PlotPrinter.print(new RateGenResultsPlotPrintable(), this);
		
		}else if(ae.getSource()==saveButton){
			
			PlotSaver.savePlot(rateGenResultsPlotCanvas, this);
		
		}else if(ae.getSource()==redrawButton){
	...
	public void redrawPlot(){
		...
		rateGenResultsPlotCanvas.setPlotState();
		rateGenResultsPlotCanvas.repaint();
...
class RateGenResultsPlotPrintable implements Printable{

	public int print(Graphics g, PageFormat pf, int pageIndex){
	
		if(pageIndex!=0){return NO_SUCH_PAGE;}

		Cina.rateGenFrame.rateGenResultsPlotFrame.rateGenResultsPlotCanvas.paintMe(g);
