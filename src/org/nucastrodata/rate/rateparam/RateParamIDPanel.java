...
/**
 * The Class RateParamIDPanel.
 */
public class RateParamIDPanel extends WizardPanel implements ItemListener, ActionListener, TreeExpansionListener{
    ...
    int[] reactionIndexArray = {3, 3, 3, 3, 3, 3, 3, 3, 3, 3};
    								
    /** The reaction name array. */
    String[] reactionNameArray = {"31Cl + p --> 32Ar"
    								, "33Cl + p --> 34Ar"
    								, "34Cl + p --> 35Ar"
    								, "34Ar + p --> 35K"
    								, "35K + p --> 36Ca"
	...
	//Constructor
	/**
	 * Instantiates a new rate param id panel.
	 *
	 * @param ds the ds
	 */
	public RateParamIDPanel(RateParamDataStructure ds){
		
		this.ds = ds;
		
		//Set the current panel index to 1 in the parent frame
		Cina.rateParamFrame.setCurrentPanelIndex(1);
		...
		if(!Cina.cinaMainDataStructure.getUser().equals("guest")
	    	...
	    	topLabel2 = new JLabel("<html>Choose a reaction type from the dropdown menu, then enter the element <p>and mass number for each reactant and product in the fields below. Choose<p>a decay type from the dropdown menu. If the reaction is not a decay, choose<p>\"none\" from the decay type menu. Add notes for this reaction in the <i>Notes</i> field.</html>");
			...
			addWizardPanel("Rate Parameterizer", "Rate Identification", "1", "12");
			...
			topLabel = new JLabel("<html>Guest users are restricted to a selected subset of rates located on our server. <p>Registered users upload rates to our server and parameterize them with <p>the Rate Parameterizer codes.</html>");
			...
			addWizardPanel("Rate Parameterizer", "Choose Tabulated Reaction Rate", "1", "12");
			...
			treeTitleLabel = new JLabel("Select reaction from the tree below.");
			...
			topLabel = new JLabel("<html>Please select a reaction rate file<p>from the tree on the left.</html>");
	...
	public void actionPerformed(ActionEvent ae){
		...
		if(ae.getSource()==removeNucDataButton){
			...
			if(Cina.rateParamFrame.rateParamChartFrame==null){
	
				Cina.rateParamFrame.rateParamChartFrame = new RateParamChartFrame(ds);
				Cina.rateParamFrame.rateParamChartFrame.setCurrentState();
				Cina.rateParamFrame.rateParamChartFrame.setVisible(true);
	...
	public void createZNodes(NucDataSetDataStructure appndsds, DefaultMutableTreeNode parentNode){
		...
		boolean[] flagArray = Cina.cinaCGIComm.doCGIComm("GET NUC DATA SET ISOTOPES", Cina.rateParamFrame);
	...
	public void createReactionNodes(NucDataSetDataStructure appndsds, DefaultMutableTreeNode parentNode, int zIndex, int aIndex){
		...
		boolean[] flagArray = Cina.cinaCGIComm.doCGIComm("GET NUC DATA LIST", Cina.rateParamFrame);
	...
	public void createDataTypeNodes(NucDataSetDataStructure appndsds, DefaultMutableTreeNode parentNode){
		...
		for(int i=0; i<appndsds.getNucDataDataStructures().length; i++){
			...
			if(appndsds.getNucDataDataStructures()[i].getNucDataType().equals("R(T)")){
			
				DefaultMutableTreeNode tempNucDataNode = new DefaultMutableTreeNode("R(T)");
		...
		if(counter==0){
			...
			DefaultMutableTreeNode tempNucDataNode = new DefaultMutableTreeNode("Reaction does not have R(T) data type");
	...
	public void createPointArrays(){
		...
		//31Cl + p --> 32Ar
		isoIn1[0] = new Point(17, 31);
		isoIn1[1] = new Point(1, 1);
		isoOut1[0] = new Point(18, 32);
		
		isoInVector.add(isoIn1);
		isoOutVector.add(isoOut1);
		
		//33Cl + p --> 34Ar
		isoIn2[0] = new Point(17, 33);
		isoIn2[1] = new Point(1, 1);
		isoOut2[0] = new Point(18, 34);
		
		isoInVector.add(isoIn2);
		isoOutVector.add(isoOut2);
		
		//34Cl + p --> 35Ar
		isoIn3[0] = new Point(17, 34);
		isoIn3[1] = new Point(1, 1);
		isoOut3[0] = new Point(18, 35);
		
		isoInVector.add(isoIn3);
		isoOutVector.add(isoOut3);
		
		//34Ar + p --> 35K
		isoIn4[0] = new Point(18, 34);
		isoIn4[1] = new Point(1, 1);
		isoOut4[0] = new Point(19, 35);
		
		isoInVector.add(isoIn4);
		isoOutVector.add(isoOut4);
		
		//35K + p --> 36Ca
		isoIn5[0] = new Point(19, 35);
		isoIn5[1] = new Point(1, 1);
		isoOut5[0] = new Point(20, 36);
		
		isoInVector.add(isoIn5);
		isoOutVector.add(isoOut5);
		
		//36K + p --> 37Ca
		isoIn6[0] = new Point(19, 36);
		isoIn6[1] = new Point(1, 1);
		isoOut6[0] = new Point(20, 37);
		
		isoInVector.add(isoIn6);
		isoOutVector.add(isoOut6);
		
		//37K + p --> 38Ca
		isoIn7[0] = new Point(19, 37);
		isoIn7[1] = new Point(1, 1);
		isoOut7[0] = new Point(20, 38);
		
		isoInVector.add(isoIn7);
		isoOutVector.add(isoOut7);
		
		//38K + p --> 39Ca
		isoIn8[0] = new Point(19, 38);
		isoIn8[1] = new Point(1, 1);
		isoOut8[0] = new Point(20, 39);
		
		isoInVector.add(isoIn8);
		isoOutVector.add(isoOut8);
		
		//39K + p --> 40Ca
		isoIn9[0] = new Point(19, 39);
		isoIn9[1] = new Point(1, 1);
		isoOut9[0] = new Point(20, 40);
		
		isoInVector.add(isoIn9);
		isoOutVector.add(isoOut9);
		
		//39K + 4He --> 43Sc
		isoIn10[0] = new Point(19, 39);
		isoIn10[1] = new Point(2, 4);
		isoOut10[0] = new Point(21, 43);
	...
	//in the panel by accessing the RateParamDataStructure object
	/**
