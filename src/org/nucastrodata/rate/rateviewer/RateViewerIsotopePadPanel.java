...
/**
 * The Class RateViewerIsotopePadPanel.
 */
public class RateViewerIsotopePadPanel extends JPanel implements MouseListener
											, MouseMotionListener, Scrollable{
    
    /** The xoffset. */
    private final int xoffset = 58;  
    
    /** The yoffset. */
    private final int yoffset = 35;
    
    /** The box width. */
    private final int boxWidth = 29;
    
    /** The box height. */
    private final int boxHeight = 29;             
    
    /** The crosshair size. */
    private final int crosshairSize = 30;
    
    /** The chart height. */
    private int xmax, ymax, biggestN, mouseX, mouseY, mouseXDrag, mouseYDrag
    				, maxPlotN, maxPlotZ, protonNumber, neutronNumber
    				, protonNumberDrag, neutronNumberDrag, chartWidth
    				, chartHeight;                                   
    ...
    private Vector protonNumberDragVector = new Vector();
    
    /** The neutron number drag vector. */
    private Vector neutronNumberDragVector = new Vector();
    
    /** The viktor. */
    private Vector viktor = new Vector();
    
    /** The viktor array. */
    private Vector[] viktorArray;
    
    /** The min drip n. */
    private int[] minDripN;
    
    /** The max drip n. */
    private int[] maxDripN;
    
    /** The Z list. */
    private int[] minZDrip, ZList;
	
	/** The isotope list. */
	private int[][] isotopeList;
	
	/** The select color. */
	private final Color selectColor = new Color(153,102,153);
	
	/** The non select color. */
	private final Color nonSelectColor = new Color(0,0,180);
	
	/** The ds. */
	private RateViewerDataStructure ds;
	
    /**
     * Instantiates a new rate viewer isotope pad panel.
     *
     * @param ds the ds
     */
    public RateViewerIsotopePadPanel(RateViewerDataStructure ds){
    	this.ds = ds;
    	setAutoscrolls(true);
        addMouseListener(this);
        addMouseMotionListener(this);
        chartWidth = (int)(boxWidth*(nmax+1));
        chartHeight = (int)(boxHeight*(zmax+1));
        xmax = (int)(xoffset + chartWidth);
        ymax = (int)(yoffset + chartHeight);
	 ...
	 * @return the nmax
	 */
	protected int getNmax(){return nmax;}
	
	/**
	 * Sets the nmax.
	 *
	 * @param nmax the new nmax
	 */
	protected void setNmax(int nmax){this.nmax = nmax;}
	 ...
	 * @return the zmax
	 */
	protected int getZmax(){return zmax;}
	
	/**
	 * Sets the zmax.
	 *
	 * @param zmax the new zmax
	 */
	protected void setZmax(int zmax){this.zmax = zmax;}
	 ...
	 */
	protected boolean getCrosshairsOn(){return crosshairsOn;}
	...
	protected void setChartSize(){
		chartWidth = (int)(boxWidth*(nmax+1));
        chartHeight = (int)(boxHeight*(zmax+1));
        setSize(chartWidth+2*xoffset,chartHeight+2*yoffset);
        xmax = (int)(xoffset + chartWidth);
        ymax = (int)(yoffset + chartHeight);
	}
	 ...
	 */
	private boolean goodIsotopeList(){
    	boolean goodIsotopeList = false;
    	int index = 0;
    
    	for(int i=0; i<ds.getNumberLibraryStructures(); i++){
			if(ds.getLibraryStructureArray()[i].getLibName().equals("ReaclibV2.2")){index = i;}
    	}
    
    	ds.setCurrentLibraryDataStructure(ds.getLibraryStructureArray()[index]);			
		ds.setLibraryName(ds.getCurrentLibraryDataStructure().getLibName());
		
		return Cina.cinaCGIComm.doCGICall("GET RATE LIBRARY ISOTOPES", Cina.rateViewerFrame);
	}
	 ...
	 * @return the z list
	 */
	private int[] getZList(){
		for(int i=0; i<ds.getNumberLibraryStructures(); i++){
			if(ds.getLibraryStructureArray()[i].getLibName().equals("ReaclibV2.2")){
				ZList = ds.getLibraryStructureArray()[i].getZList();
			}
		}
	 ...
	 */
	private int[][] getIsotopeList(){
	
		for(int i=0; i<ds.getNumberLibraryStructures(); i++){
			if(ds.getLibraryStructureArray()[i].getLibName().equals("ReaclibV2.2")){
				isotopeList = ds.getLibraryStructureArray()[i].getIsotopeList();
			}
		}
	 ...
	 */
	private int[] getMinDripN(){
	
		int[] tempArray = new int[ZList.length];
		//Subtract here to get neutron number from A values
		for(int i=0; i<ZList.length; i++){tempArray[i] = isotopeList[i][0] - ZList[i];}
	...
	private int[] getMaxDripN(){
		...
		for(int i=0; i<ZList.length; i++){
			...
			for(int j=0; j<isotopeList[i].length; j++){
				//Check for -1 entry cause -1 is entered when an proton list entry has no more listed isotopes 
				if(isotopeList[i][j]!=-1){lastIndex = j;}
			}		
	...
	protected void setCurrentState(){
		initPStable();
		viktor = ds.getViktorArray()[ds.getLibIndex()];
		repaint();
	}
	...
	protected void getCurrentState(){
		...
		viktorArray[ds.getLibIndex()] = viktor;
    ...
    private boolean doIBelong(int P, int N){
        ...
        if(P>=0 && P<=zmax){
            if(N>=minDripN[P] && N<=maxDripN[P]){include = true;}
        }
    ...
    public void mousePressed(MouseEvent me){
        ...
        if(!viktor.contains(new Point((int)protonNumber, (int)neutronNumber))
        	&& Cina.rateViewerFrame.containsIsotope((int)protonNumber, (int)neutronNumber)){
		...
		for(int i=0; i<ds.getViktorArray().length; i++){
			if(ds.getViktorArray()[i].size()!=0){empty = false;}
		}
		
		boolean checked = false;
		
		for(int i=0; i<8; i++){
			if(Cina.rateViewerFrame.checkBoxArray[i].isSelected()){checked = true;}
		}
		
		if(empty || !checked){
			Cina.rateViewerFrame.rateButton.setEnabled(false);
		}else{
			Cina.rateViewerFrame.rateButton.setEnabled(true);
		}
    ...
    public void mouseEntered(MouseEvent me){
    	...
    	Cina.rateViewerFrame.ZRuler.setPreferredWidth((int)getSize().getWidth());
       	Cina.rateViewerFrame.NRuler.setPreferredHeight((int)getSize().getHeight());
       	
    	Cina.rateViewerFrame.ZRuler.setCurrentState(zmax, nmax, mouseX, mouseY, xoffset, yoffset, crosshairsOn);
    	Cina.rateViewerFrame.NRuler.setCurrentState(zmax, nmax, mouseX, mouseY, xoffset, yoffset, crosshairsOn);
    ...
    public void mouseExited(MouseEvent me){
    	...
    	Cina.rateViewerFrame.elementField.setText("");
    	Cina.rateViewerFrame.zField.setText("");	
    	Cina.rateViewerFrame.aField.setText("");
	...
	public void mouseReleased(MouseEvent me){
		...
		for(int i=0; i<protonNumberDragVector.size(); i++){
			...
			for(int j=0; j<neutronNumberDragVector.size(); j++){
				...
				if(!viktor.contains(new Point(z, n)) && Cina.rateViewerFrame.containsIsotope(z, n)){
	...
	public void mouseMoved(MouseEvent me){
        ...
        if((int)protonNumber>=0 && (int)neutronNumber>=1){
	        Cina.rateViewerFrame.zField.setText(String.valueOf((int)protonNumber));	
	        Cina.rateViewerFrame.elementField.setText(Cina.cinaMainDataStructure.getElementSymbol((int)protonNumber));
	    	Cina.rateViewerFrame.aField.setText(String.valueOf((int)neutronNumber + (int)protonNumber));
    	}else{
    		Cina.rateViewerFrame.elementField.setText("");
	    	Cina.rateViewerFrame.zField.setText("");	
	    	Cina.rateViewerFrame.aField.setText("");
	 ...
	 * @return the nZ point
	 */
	private Point getNZPoint(int x, int y) {
	 ...
	 * @return the drag nz
	 */
	private void getDragNZ(int x, int y) {
    ...
    public void paintComponent(Graphics g){
        ...
        FontMetrics smallFontMetrics = getFontMetrics(Fonts.smallFont);
	    FontMetrics realSmallFontMetrics = getFontMetrics(Fonts.realSmallFont);
	    FontMetrics tinyFontMetrics = getFontMetrics(Fonts.tinyFont);
        ...
        for(int z=0; z<=zmax; z++){
            for(int n=minDripN[z]; n<=(int)Math.min((long)maxDripN[z],(long)nmax); n++){
                if(!doIBelong(z, n)){continue;}

                boolean overIsotope = false;
                
                if(z==protonNumber && n==neutronNumber && crosshairsOn){overIsotope = true;}
                ...
                if(Cina.rateViewerFrame.containsIsotope((int)z, (int)n)){
	                if(viktor.contains(new Point((int)z, (int)n))){
	                	g2.setColor(selectColor);   
	                }else if(!viktor.contains(new Point((int)z, (int)n)) && !overIsotope){
        ...
        for(int z=0; z<=zmax-1; z++){
           ...
           if(tempS.equals("H")){
            	int ds = (int)(minDripN[zmax-z]*boxWidth);
                g2.drawString(tempS,
        ...
        for(int n=0; n<=maxPlotN; n++){
           ...
           g2.drawString(tempS,
                        ...
                        yoffset+chartHeight + 17 - minZDrip[n]*boxHeight);
        }
    ...
    public int getScrollableUnitIncrement(Rectangle visibleRect,
        ...
        if(direction < 0){
            int newPosition = currentPosition - (currentPosition/29)*29;
            return (newPosition == 0) ? 29: newPosition;
        }else{
            return((currentPosition/29)+1)*29 - currentPosition;
        }
    }
