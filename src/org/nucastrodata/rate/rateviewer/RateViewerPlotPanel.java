...
/**
 * The Class RateViewerPlotPanel.
 */
public class RateViewerPlotPanel extends PlotPanel implements MouseMotionListener
																, MouseListener{
	...
	/**
	 * Instantiates a new rate viewer plot panel.
	 *
	 ...
	 * @param ds the ds
	 */
	public RateViewerPlotPanel(int rminInit
								, int rmaxInit
								, int tminInit
								, int tmaxInit
								, RateViewerDataStructure ds){
	...
	public void setPlotState(){	
		...
		int numberRates = ds.getNumberTotalRates()
							+ ds.getNumberCompRates();
							
		int length = ds.getTempGrid().length;
			
		if(ds.getRateAdded()){
			numberRates++;	
			
			if(ds.getAddTempArray().length>length){
				length = ds.getAddTempArray().length;
			}
		}
	
		imax = numberRates;
		mode = new int[numberRates];					
		doplot = new int[numberRates];					
		curveTitle = new String[numberRates];
		
		x = new double[numberRates][length];
		y = new double[numberRates][length];
		...
		for(int i=0; i<ds.getNumberLibraryStructures(); i++){
			if(ds.getLibraryStructureArray()[i].getRateDataStructures()!=null){
				for(int j=0; j<ds.getLibraryStructureArray()[i].getRateDataStructures().length; j++){
					x[numberRates] = ds.getLibraryStructureArray()[i].getRateDataStructures()[j].getTempDataArrayTotal();
					y[numberRates] = ds.getLibraryStructureArray()[i].getRateDataStructures()[j].getRateDataArrayTotal();
					...
					if(ds.getLibraryStructureArray()[i].getRateDataStructures()[j].getResonantInfo().length>1){
						for(int k=0; k<ds.getLibraryStructureArray()[i].getRateDataStructures()[j].getResonantInfo().length; k++){
							...
							x[numberRates] = ds.getLibraryStructureArray()[i].getRateDataStructures()[j].getTempDataArrayComp()[k];
							y[numberRates] = ds.getLibraryStructureArray()[i].getRateDataStructures()[j].getRateDataArrayComp()[k];
		...
		if(ds.getRateAdded()){
			...
			x[numberRates] = ds.getAddTempArray();
			y[numberRates] = ds.getAddRateArray();
		...
		if(initFlag){
			plotmode = Cina.rateViewerFrame.rateViewerPlotFrame.getPlotMode();
		
			xmin = Math.pow(10, Cina.rateViewerFrame.rateViewerPlotFrame.getTmin());
			xmax = Math.pow(10, Cina.rateViewerFrame.rateViewerPlotFrame.getTmax());
			ymin = Math.pow(10, Cina.rateViewerFrame.rateViewerPlotFrame.getRmin());
			ymax = Math.pow(10, Cina.rateViewerFrame.rateViewerPlotFrame.getRmax());
			
			minorX = Cina.rateViewerFrame.rateViewerPlotFrame.getMinorT();
			majorX = Cina.rateViewerFrame.rateViewerPlotFrame.getMajorT();			
			minorY = Cina.rateViewerFrame.rateViewerPlotFrame.getMinorR();
			majorY = Cina.rateViewerFrame.rateViewerPlotFrame.getMajorR();
			
			xtickIntervals = Math.abs((int)Cina.rateViewerFrame.rateViewerPlotFrame.getTmin()
									- (int)Cina.rateViewerFrame.rateViewerPlotFrame.getTmax());
			ytickIntervals = Math.abs((int)Cina.rateViewerFrame.rateViewerPlotFrame.getRmin()
									- (int)Cina.rateViewerFrame.rateViewerPlotFrame.getRmax());
			...
			for(int i=0; i<numberRates; i++){
				...
				if(ds.getRateAdded()){
					...
					if(i!=numberRates-1){
						...
						if(Cina.rateViewerFrame.rateViewerPlotFrame.rateViewerReactionListPanel.checkBoxArray[i].getText().indexOf("(nr")!=-1
							|| Cina.rateViewerFrame.rateViewerPlotFrame.rateViewerReactionListPanel.checkBoxArray[i].getText().indexOf("(r")!=-1){	
							mode[i] = 8;
						}else{
							mode[i] = 1;
						}
		
						curveTitle[i] = Cina.rateViewerFrame.rateViewerPlotFrame.rateViewerReactionListPanel.checkBoxArray[i].getText() 
										+ " "
										+ ds.getUnitsArray()[i];
					...
					if(Cina.rateViewerFrame.rateViewerPlotFrame.rateViewerReactionListPanel.checkBoxArray[i].isSelected()){
						doplot[i] = 1;
						if(Cina.rateViewerFrame.rateViewerPlotFrame.legendBox.isSelected()){
							showLegend = true;
						}
					}else{
						doplot[i] = 0;
		...
		for(int i=0; i<npoints.length; i++){
			...
			if(ds.getRateAdded()){
				...
				if(i!=npoints.length-1){
					...
					npoints[i] = ds.getTempGrid().length;
				
				}else{
				
					npoints[i] = ds.getAddTempArray().length;
